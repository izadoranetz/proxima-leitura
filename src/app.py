...
def my_books_page():
    ...
    if saved_books:
        ...
        for idx, bid in enumerate(saved_books):
            ...
            with cols[idx % 3]:
                st.markdown(f"<div style='border:1px solid #eee; border-radius:10px; padding:10px; margin:4px 0; background:#fafafa;'>", unsafe_allow_html=True)
                ...
                st.markdown("<div style='margin-top:10px; margin-bottom:5px; font-size:16px; font-weight:bold;'>Sua avaliação:</div>", unsafe_allow_html=True)
...
def home_page():
    ...
    if page == "Preferências":
        ...
        if q:
            ql = q.lower()
            mask = df_explore['title'].str.lower().str.contains(ql, na=False) | \
